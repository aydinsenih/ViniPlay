...
let detectedHardware = {
  ...
  intel_vaapi: null,
  radeon_vaapi: null,
}; // MODIFIED: To store specific Intel GPU info
...
console.log(
  `[INIT] Application starting. Data directory: ${DATA_DIR}, Public directory: ${PUBLIC_DIR}`
);
...
try {
  ...
  console.error(
    `[INIT] FATAL: Failed to create necessary directories: ${mkdirError.message}`
  );
  process.exit(1);
}
...
const db = new sqlite3.Database(DB_PATH, (err) => {
  if (err) {
    ...
    db.serialize(() => {
      db.run(
        `CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT, isAdmin INTEGER DEFAULT 0, canUseDvr INTEGER DEFAULT 0)`,
        (err) => {
          if (err) {
      ...
      db.run(
        `CREATE TABLE IF NOT EXISTS user_settings (user_id INTEGER NOT NULL, key TEXT NOT NULL, value TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, PRIMARY KEY (user_id, key))`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS multiview_layouts (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, name TEXT NOT NULL, layout_data TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, channelId TEXT NOT NULL, channelName TEXT NOT NULL, channelLogo TEXT, programTitle TEXT NOT NULL, programDesc TEXT, programStart TEXT NOT NULL, programStop TEXT NOT NULL, notificationTime TEXT NOT NULL, programId TEXT NOT NULL, status TEXT DEFAULT 'pending', triggeredAt TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS push_subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, endpoint TEXT UNIQUE NOT NULL, p256dh TEXT NOT NULL, auth TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`
      );
      db.run(
      ...
      db.run(
        `CREATE TABLE IF NOT EXISTS dvr_jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, channelId TEXT NOT NULL, channelName TEXT NOT NULL, programTitle TEXT NOT NULL, startTime TEXT NOT NULL, endTime TEXT NOT NULL, status TEXT NOT NULL, ffmpeg_pid INTEGER, filePath TEXT, profileId TEXT, userAgentId TEXT, preBufferMinutes INTEGER, postBufferMinutes INTEGER, errorMessage TEXT, isConflicting INTEGER DEFAULT 0, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`
      );
      db.run(
        `CREATE TABLE IF NOT EXISTS dvr_recordings (id INTEGER PRIMARY KEY AUTOINCREMENT, job_id INTEGER, user_id INTEGER NOT NULL, channelName TEXT NOT NULL, programTitle TEXT NOT NULL, startTime TEXT NOT NULL, durationSeconds INTEGER, fileSizeBytes INTEGER, filePath TEXT UNIQUE NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (job_id) REFERENCES dvr_jobs(id) ON DELETE SET NULL)`
      );

      //-- ENHANCEMENT: Modify stream history table to include more data for the admin panel.
      db.run(
        `CREATE TABLE IF NOT EXISTS stream_history (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, username TEXT NOT NULL, channel_id TEXT, channel_name TEXT, start_time TEXT NOT NULL, end_time TEXT, duration_seconds INTEGER, status TEXT NOT NULL, client_ip TEXT, channel_logo TEXT, stream_profile_name TEXT)`,
        (err) => {
          if (!err) {
            // Add new columns non-destructively if the table already exists
            db.run(
              "ALTER TABLE stream_history ADD COLUMN channel_logo TEXT",
              () => {}
            );
            db.run(
              "ALTER TABLE stream_history ADD COLUMN stream_profile_name TEXT",
              () => {}
            );
          }
        }
...
app.use((req, res, next) => {
  ...
  const user_info = req.session.userId
    ? `User ID: ${req.session.userId}, Admin: ${req.session.isAdmin}, DVR: ${req.session.canUseDvr}`
    : "No session";
  console.log(
    `[HTTP_TRACE] ${req.method} ${req.originalUrl} - IP: ${req.clientIp} - Session: [${user_info}]`
  );
  next();
});
...
const requireAuth = (req, res, next) => {
  ...
  db.get(
    ...
    (err, user) => {
      if (err) {
        console.error(
          "[AUTH_MIDDLEWARE] DB error checking user existence:",
          err
        );
        return res
          .status(500)
      ...
      if (!user) {
        console.warn(
          `[AUTH_MIDDLEWARE] User ID ${req.session.userId} from session not found in DB. Destroying session.`
        );
        req.session.destroy();
...
const requireDvrAccess = (req, res, next) => {
  if (req.session && (req.session.canUseDvr || req.session.isAdmin))
    return next();
...
async function detectHardwareAcceleration() {
  ...
  exec("vainfo --display drm", (err, stdout, stderr) => {
    if (err) {
      console.log(
        "[HW] VA-API Radeon/AMD not detected or vainfo command failed."
      );
...
function cleanupInactiveStreams() {
  const now = Date.now();
  console.log(
    `[JANITOR] Running cleanup for inactive streams. Current active processes: ${activeStreamProcesses.size}`
  );

  activeStreamProcesses.forEach((streamInfo, streamKey) => {
    if (
      streamInfo.references <= 0 &&
      now - streamInfo.lastAccess > STREAM_INACTIVITY_TIMEOUT
    ) {
      console.log(
        `[JANITOR] Found stale stream process for key: ${streamKey}. Terminating PID: ${streamInfo.process.pid}.`
      );
      try {
        ...
        if (streamInfo.historyId) {
          ...
          const duration = Math.round(
            (new Date(endTime).getTime() -
              new Date(streamInfo.startTime).getTime()) /
              1000
          );
          db.run(
            "UPDATE stream_history SET end_time = ?, duration_seconds = ?, status = 'stopped' WHERE id = ? AND status = 'playing'",
        ...
        console.warn(
          `[JANITOR] Error killing stale process for ${streamKey}: ${e.message}`
        );
        activeStreamProcesses.delete(streamKey);
...
function sendSseEvent(userId, eventName, data) {
  ...
  if (clients && clients.length > 0) {
    console.log(
      `[SSE] Sending event '${eventName}' to ${clients.length} client(s) for user ID ${userId}.`
    );
...
function broadcastAdminUpdate() {
  ...
  const transcodedLive = Array.from(activeStreamProcesses.values()).map(
    (info) => ({
  ...
  const redirectLive = Array.from(activeRedirectStreams.values()).map(
    (info) => ({
  ...
  console.log(
    `[SSE_ADMIN] Broadcasted combined activity update (${combinedLiveActivity.length} live streams) to all connected admins.`
  );
}
...
function broadcastSseToAll(eventName, data) {
  ...
  console.log(
    `[SSE_BROADCAST] Broadcasted event '${eventName}' to ${clientCount} total clients.`
  );
}
...
function getSettings() {
  const defaultSettings = {
    ...
    userAgents: [
      {
        id: `default-ua-1724778434000`,
        name: "ViniPlay Default",
        value: "VLC/3.0.20 (Linux; x86_64)",
        isDefault: true,
      },
    ],
    streamProfiles: [
      {
        id: "ffmpeg-default",
        name: "ffmpeg (Built in)",
        command:
          '-user_agent "{userAgent}" -i "{streamUrl}" -c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 128k -f mpegts pipe:1',
        isDefault: true,
      },
      // FINAL FIX: This is the modern, more compatible command for NVIDIA streaming.
      {
        id: "ffmpeg-nvidia",
        name: "ffmpeg (NVIDIA NVENC)",
        command:
          '-user_agent "{userAgent}" -re -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a copy -f mpegts pipe:1',
        isDefault: false,
      },
      {
        id: "ffmpeg-nvidia-legacy",
        name: "ffmpeg (NVIDIA NVENC - Legacy)",
        command:
          '-user_agent "{userAgent}" -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a aac -b:a 128k -f mpegts pipe:1',
        isDefault: false,
      },
      {
        id: "ffmpeg-intel",
        name: "ffmpeg (Intel QSV)",
        command:
          '-hwaccel qsv -c:v h264_qsv -i "{streamUrl}" -c:v h264_qsv -preset medium -c:a aac -b:a 128k -f mpegts pipe:1',
        isDefault: false,
      },
      // NEW: Add this line for VA-API
      {
        ...
        name: "ffmpeg (VA-API)",
        command:
          "-hwaccel vaapi -hwaccel_output_format vaapi -i \"{streamUrl}\" -vf 'format=nv12,hwupload' -c:v h264_vaapi -preset medium -c:a aac -b:a 128k -f mpegts pipe:1",
        isDefault: false,
      },
      {
        id: "ffmpeg-vaapi-amd",
        name: "ffmpeg (VA-API) Radeon/AMD",
        command:
          '-vaapi_device /dev/dri/renderD128 -hwaccel vaapi -hwaccel_output_format vaapi -i "{streamUrl}" -c:v h264_vaapi -c:a aac -b:a 128k -f mpegts pipe:1',
        isDefault: false,
      },
      {
        id: "ffmpeg-nvidia-reconnect",
        name: "ffmpeg (NVIDIA reconnect)",
        command:
          '-user_agent "{userAgent}" -re -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a copy -f mpegts pipe:1',
        isDefault: false,
      },
      {
        id: "redirect",
        name: "Redirect (No Transcoding)",
        command: "redirect",
        isDefault: false,
      },
    ],
    dvr: {
      ...
      recordingProfiles: [
        ...
        {
          id: "dvr-ts-default",
          name: "Default TS (Stream Copy, Timeshiftable)",
          command:
            '-user_agent "{userAgent}" -i "{streamUrl}" -c copy -f mpegts "{filePath}"',
          isDefault: true,
        },
        ...
        {
          id: "dvr-ts-nvidia",
          name: "NVIDIA NVENC TS (Timeshiftable)",
          command:
            '-user_agent "{userAgent}" -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a copy -f mpegts "{filePath}"',
          isDefault: false,
        },
        {
          id: "dvr-ts-nvidia-reconnect",
          name: "NVIDIA NVENC TS reconnect",
          command:
            '-user_agent "{userAgent}" -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a copy -f mpegts "{filePath}"',
          isDefault: false,
        },
        ...
        {
          id: "dvr-mp4-default",
          name: "Legacy MP4 (H.264/AAC)",
          command:
            '-user_agent "{userAgent}" -i "{streamUrl}" -c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 128k -movflags +faststart -f mp4 "{filePath}"',
          isDefault: false,
        },
        {
          id: "dvr-mp4-nvidia",
          name: "NVIDIA NVENC MP4 (H.264/AAC)",
          command:
            '-user_agent "{userAgent}" -i "{streamUrl}" -c:v h264_nvenc -preset p6 -tune hq -c:a aac -b:a 128k -movflags +faststart -f mp4 "{filePath}"',
          isDefault: false,
        },
        {
          id: "dvr-mp4-intel",
          name: "Intel QSV MP4 (H.264/AAC)",
          command:
            '-hwaccel qsv -c:v h264_qsv -i "{streamUrl}" -c:v h264_qsv -preset medium -c:a aac -b:a 128k -movflags +faststart -f mp4 "{filePath}"',
          isDefault: false,
        },
        // NEW: Add this line for VA-API recording
        {
          id: "dvr-mp4-vaapi",
          name: "VA-API MP4 (H.264/AAC)",
          command:
            '-hwaccel vaapi -hwaccel_output_format vaapi -i "{streamUrl}" -vf \'format=nv12,hwupload\' -c:v h264_vaapi -preset medium -c:a aac -b:a 128k -movflags +faststart -f mp4 "{filePath}"',
          isDefault: false,
        },
        {
          id: "dvr-mp4-radeon-vaapi",
          name: "Radeon/AMD VA-API MP4 (H.264/AAC)",
          command:
            '-vaapi_device /dev/dri/renderD128 -hwaccel vaapi -hwaccel_output_format vaapi -i "{streamUrl}" -c:v h264_vaapi -preset medium -c:a aac -b:a 128k -movflags +faststart -f mp4 "{filePath}"',
          isDefault: false,
        },
      ],
    },
  ...
  if (!fs.existsSync(SETTINGS_PATH)) {
    console.log(
      "[SETTINGS] settings.json not found, creating default settings."
    );
  ...
  try {
    ...
    defaultSettings.streamProfiles.forEach((defaultProfile) => {
      const existingProfile = settings.streamProfiles.find(
        (p) => p.id === defaultProfile.id
      );
      if (!existingProfile) {
        console.log(
          `[SETTINGS_MIGRATE] Adding missing stream profile: ${defaultProfile.name}`
        );
        ...
        if (existingProfile.command !== defaultProfile.command) {
          console.log(
            `[SETTINGS_MIGRATE] Updating outdated default stream profile command for: ${defaultProfile.name}`
          );
    ...
    if (!settings.dvr) {
      ...
      defaultSettings.dvr.recordingProfiles.forEach((defaultProfile) => {
        const existingProfile = settings.dvr.recordingProfiles.find(
          (p) => p.id === defaultProfile.id
        );
        if (!existingProfile) {
          console.log(
            `[SETTINGS_MIGRATE] Adding missing DVR recording profile: ${defaultProfile.name}`
          );
          ...
          if (existingProfile.command !== defaultProfile.command) {
            console.log(
              `[SETTINGS_MIGRATE] Updating outdated default DVR profile command for: ${defaultProfile.name}`
            );
            existingProfile.command = defaultProfile.command;
            needsSave = true;
          }
    ...
    if (needsSave) {
      console.log(
        "[SETTINGS_MIGRATE] Saving updated settings file after migration."
      );
    ...
    console.error(
      "[SETTINGS] Could not parse settings.json, returning default. Error:",
      e.message
    );
    return defaultSettings;
  }
...
function fetchUrlContent(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log(
      `[FETCH] Attempting to fetch URL content: ${url} (Timeout: ${
      ...
      (res) => {
        ...
        if (
          ...
          return fetchUrlContent(
            ...
            options
          ).then(resolve, reject); // <-- MODIFIED
        }
        if (res.statusCode !== 200) {
          console.error(
            `[FETCH] Failed to fetch ${url}: Status Code ${res.statusCode}`
          );
          return reject(
    ...
    request.on("timeout", () => {
      ...
      const timeoutError = new Error(
        `Request to ${url} timed out after ${TIMEOUT_DURATION / 1000} seconds.`
      );
...
const parseEpgTime = (timeStr, offsetHours = 0) => {
  ...
  if (sign && tzHours && tzMinutes) {
    const epgOffsetMinutes =
      (parseInt(tzHours) * 60 + parseInt(tzMinutes)) * (sign === "+" ? 1 : -1);
    date = new Date(
      Date.UTC(year, parseInt(month) - 1, day, hours, minutes, seconds)
    );
...
async function processAndMergeSources(req) {
  ...
  for (const source of activeM3uSources) {
    console.log(
      `[M3U] Processing source: "${source.name}" (ID: ${source.id}, Type: ${source.type}, Path: ${source.path})`
    );
    sendProcessingStatus(
      req,
      `Processing M3U source: "${source.name}"...`,
      "info"
    ); // <-- NEW
    try {
      let content = "";
      ...
      if (source.type === "file") {
        ...
        sendProcessingStatus(
          req,
          ` -> Successfully fetched M3U content.`,
          "info"
        ); // <-- NEW
        ...
        if (!server || !username || !password) {
          throw new Error(
            "XC source is missing server, username, or password."
          );
        }
        ...
        sendProcessingStatus(
          req,
          ` -> Fetching content from XC server...`,
          "info"
        ); // <-- NEW
        ...
        sendProcessingStatus(
          req,
          ` -> Successfully fetched M3U content from XC server.`,
          "info"
        ); // <-- NEW
        ...
        if (!activeEpgSources.some((s) => s.id === epgSource.id)) {
          ...
          sendProcessingStatus(
            req,
            ` -> Automatically added EPG source for "${source.name}".`,
            "info"
          ); // <-- NEW
        }
      }
        ...
        if (line.startsWith("#EXTINF:")) {
          ...
          if (tvgIdMatch) {
            processedAttributes = processedAttributes.replace(
              /tvg-id="[^"]*"/,
              `tvg-id="${uniqueChannelId}"`
            );
          } else {
            const extinfEnd = processedAttributes.indexOf(" ") + 1;
            processedAttributes =
              processedAttributes.slice(0, extinfEnd) +
              `tvg-id="${uniqueChannelId}" ` +
              processedAttributes.slice(extinfEnd);
          }
      ...
      sendProcessingStatus(
        req,
        ` -> Processed ${streamCount} streams from "${source.name}".`,
        "info"
      ); // <-- NEW
    } catch (error) {
      ...
      sendProcessingStatus(req, `Error: ${errorMsg}`, "error"); // <-- NEW
      source.status = "Error";
      source.statusMessage = `Processing failed: ${error.message.substring(
        0,
        100
      )}...`;
    }
  ...
  try {
    ...
    sendProcessingStatus(
      req,
      `Successfully merged all M3U sources.`,
      "success"
    ); // <-- NEW
  } catch (writeErr) {
    ...
    sendProcessingStatus(
      req,
      `Error writing merged M3U file: ${writeErr.message}`,
      "error"
    ); // <-- NEW
  }
  ...
  for (const source of activeEpgSources) {
    ...
    console.log(
      `[EPG] Processing source: "${source.name}" (ID: ${source.id}, Type: ${source.type}, Path: ${source.path})`
    );
    sendProcessingStatus(
      req,
      `Processing EPG source: "${source.name}"...`,
      "info"
    ); // <-- NEW
    try {
      let xmlString = "";
      ...
      if (source.type === "file") {
        ...
        sendProcessingStatus(req, ` -> Fetching content from URL...`, "info"); // <-- NEW
        xmlString = await fetchUrlContent(
          source.path,
          source.fetchOptions || {}
        );
        sendProcessingStatus(
          req,
          ` -> Successfully fetched EPG content.`,
          "info"
        ); // <-- NEW
        try {
          fs.writeFileSync(epgFilePath, xmlString);
          console.log(
            `[EPG] Downloaded EPG for "${source.name}" saved to ${epgFilePath}.`
          );
        } catch (writeErr) {
          console.error(
            `[EPG] Error saving EPG file from URL for "${source.name}": ${writeErr.message}`
          );
        }
      }
      ...
      const programs =
        epgJson.tv && epgJson.tv.programme
          ? [].concat(epgJson.tv.programme)
          : [];
      ...
      if (programs.length === 0) {
        ...
        sendProcessingStatus(
          req,
          `Warning: No programs found in "${source.name}".`,
          "info"
        );
      }

      const m3uSourceProviders = settings.m3uSources.filter(
        (m3u) => m3u.isActive
      );
      ...
      for (const prog of programs) {
        ...
        for (const m3uSource of m3uSourceProviders) {
          ...
          const titleNode =
            prog.title && prog.title._cdata
              ? prog.title._cdata
              : prog.title?._text || "No Title";
          const descNode =
            prog.desc && prog.desc._cdata
              ? prog.desc._cdata
              : prog.desc?._text || "";

          mergedProgramData[uniqueChannelId].push({
            start: parseEpgTime(
              prog._attributes.start,
              timezoneOffset
            ).toISOString(),
            stop: parseEpgTime(
              prog._attributes.stop,
              timezoneOffset
            ).toISOString(),
      ...
      if (!source.isXcEpg) {
        ...
        console.log(
          `[EPG] Source "${source.name}" processed successfully from ${source.path}.`
        );
      }
      sendProcessingStatus(
        req,
        ` -> Processed ${programCount} programs from "${source.name}".`,
        "info"
      ); // <-- NEW
    } catch (error) {
      const errorMsg = `Failed to process source "${source.name}" from ${source.path}: ${error.message}`; // <-- NEW
  ...
  for (const channelId in mergedProgramData) {
    mergedProgramData[channelId].sort(
  ...
  try {
    ...
    console.log(
      `[EPG] Merged EPG JSON content saved to ${MERGED_EPG_JSON_PATH}.`
    );
    sendProcessingStatus(
      req,
      `Successfully merged all EPG sources.`,
      "success"
    ); // <-- NEW
  } catch (writeErr) {
    console.error(
      `[EPG] Error writing merged EPG JSON file: ${writeErr.message}`
    );
    sendProcessingStatus(
      req,
      `Error writing merged EPG JSON file: ${writeErr.message}`,
      "error"
    ); // <-- NEW
  }
  ...
  console.log(
    `[PROCESS] Finished processing. New 'sourcesLastUpdated' timestamp: ${settings.sourcesLastUpdated}`
  );
  sendProcessingStatus(
    req,
...
const updateAndScheduleSourceRefreshes = () => {
  ...
  allSources.forEach((source) => {
    if (source.type === "url" && source.isActive && source.refreshHours > 0) {
      ...
      console.log(
        `[SCHEDULER] Scheduling refresh for "${source.name}" (ID: ${source.id}) every ${source.refreshHours} hours.`
      );

      const scheduleNext = () => {
        const timeoutId = setTimeout(async () => {
          console.log(
            `[SCHEDULER_RUN] Auto-refresh triggered for "${source.name}".`
          );
          try {
            ...
            if (result.success) {
              fs.writeFileSync(
                SETTINGS_PATH,
                JSON.stringify(result.updatedSettings, null, 2)
              );
              console.log(
                `[SCHEDULER_RUN] Successfully refreshed and processed sources for "${source.name}".`
              );
            }
          } catch (error) {
            console.error(
              `[SCHEDULER_RUN] Auto-refresh for "${source.name}" failed:`,
              error.message
            );
          }
          scheduleNext();
  ...
  for (const [sourceId, timeoutId] of sourceRefreshTimers.entries()) {
    if (!activeUrlSources.has(sourceId)) {
      console.log(
        `[SCHEDULER] Clearing obsolete refresh timer for source ID: ${sourceId}`
      );
  ...
  console.log(
    `[SCHEDULER] Finished scheduling. Active timers: ${sourceRefreshTimers.size}`
  );
};
...
app.get("/api/auth/needs-setup", (req, res) => {
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        console.error(
          "[AUTH_API] Error checking admin user count:",
          err.message
        );
      ...
      console.log(
        `[AUTH_API] Admin user count: ${row.count}. Needs setup: ${needsSetup}`
      );
      res.json({ needsSetup });
    }
  );
...
app.post("/api/auth/setup-admin", (req, res) => {
  ...
  db.get("SELECT COUNT(*) as count FROM users", [], (err, row) => {
    if (err) {
      console.error(
        "[AUTH_API] Error checking user count during admin setup:",
        err.message
      );
      return res.status(500).json({ error: err.message });
    }
    if (row.count > 0) {
      console.warn(
        "[AUTH_API] Setup attempted but users already exist. Denying setup."
      );
      return res
        .status(403)
    ...
    if (!username || !password) {
      console.warn(
        "[AUTH_API] Admin setup failed: Username and/or password missing."
      );
      return res
        .status(400)
    ...
    bcrypt.hash(password, saltRounds, (err, hash) => {
      if (err) {
        console.error(
          "[AUTH_API] Error hashing password during admin setup:",
          err
        );
        return res.status(500).json({ error: "Error hashing password." });
      }
      db.run(
        "INSERT INTO users (username, password, isAdmin, canUseDvr) VALUES (?, ?, 1, 1)",
        [username, hash],
        function (err) {
          if (err) {
            console.error(
              "[AUTH_API] Error inserting admin user:",
              err.message
            );
            return res.status(500).json({ error: err.message });
          }
          ...
          console.log(
            `[AUTH_API] Admin user "${username}" created successfully (ID: ${this.lastID}). Session set.`
          );
          res.json({
            success: true,
            user: {
...
app.post("/api/auth/login", (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE username = ?", [username], (err, user) => {
    if (err) {
      console.error(
        "[AUTH_API] Error querying user during login:",
        err.message
      );
      return res.status(500).json({ error: err.message });
    }
    if (!user) {
      console.warn(
        `[AUTH_API] Login failed for username "${username}": User not found.`
      );
    ...
    bcrypt.compare(password, user.password, (err, result) => {
      ...
      if (result) {
        ...
        console.log(
          `[AUTH_API] User "${username}" (ID: ${user.id}) logged in successfully. Session set.`
        );
        res.json({
          success: true,
          user: {
            ...
            canUseDvr: user.canUseDvr === 1,
          },
        });
      } else {
        console.warn(
          `[AUTH_API] Login failed for username "${username}": Incorrect password.`
        );
...
app.post("/api/auth/logout", (req, res) => {
  ...
  req.session.destroy((err) => {
    if (err) {
      console.error(
        `[AUTH_API] Error destroying session for user ${username}:`,
        err
      );
...
app.get("/api/auth/status", (req, res) => {
  console.log(
    `[AUTH_API] GET /api/auth/status - Checking session ID: ${req.sessionID}`
  );
  if (req.session && req.session.userId) {
    console.log(
      `[AUTH_API_STATUS] Valid session found for user "${req.session.username}" (ID: ${req.session.userId}). Responding with isLoggedIn: true.`
    );
    res.json({
      isLoggedIn: true,
      user: {
    ...
    console.log(
      "[AUTH_API_STATUS] No valid session found. Responding with isLoggedIn: false."
    );
...
app.get("/api/users", requireAdmin, (req, res) => {
  ...
  db.all(
    "SELECT id, username, isAdmin, canUseDvr FROM users ORDER BY username",
    [],
    (err, rows) => {
      if (err) {
...
app.post("/api/users", requireAdmin, (req, res) => {
  ...
  if (!username || !password) {
    console.warn(
      "[USER_API] Add user failed: Username and/or password missing."
    );
    return res
      .status(400)
      .json({ error: "Username and password are required." });
  ...
  bcrypt.hash(password, saltRounds, (err, hash) => {
    ...
    db.run(
      ...
      [username, hash, isAdmin ? 1 : 0, canUseDvr ? 1 : 0],
      function (err) {
        if (err) {
        ...
        console.log(
          `[USER_API] User "${username}" added successfully (ID: ${this.lastID}).`
        );
...
app.put("/api/users/:id", requireAdmin, (req, res) => {
  ...
  const { username, password, isAdmin, canUseDvr } = req.body;
  console.log(
    `[USER_API] Updating user ID: ${id}. Username: ${username}, IsAdmin: ${isAdmin}, CanUseDvr: ${canUseDvr}`
  );
  ...
  const updateUser = () => {
    if (password) {
      bcrypt.hash(password, saltRounds, (err, hash) => {
        if (err) {
          console.error(
            "[USER_API] Error hashing password during user update:",
            err
          );
          return res.status(500).json({ error: "Error hashing password" });
        }
        db.run(
          "UPDATE users SET username = ?, password = ?, isAdmin = ?, canUseDvr = ? WHERE id = ?",
          [username, hash, isAdmin ? 1 : 0, canUseDvr ? 1 : 0, id],
          (err) => {
            if (err) {
              console.error(
                `[USER_API] Error updating user ${id} with new password:`,
                err.message
              );
              return res.status(500).json({ error: err.message });
            }
            if (req.session.userId == id) {
              ...
              console.log(
                `[USER_API] Current user's session (ID: ${id}) updated.`
              );
            }
            console.log(
              `[USER_API] User ${id} updated successfully (with password change).`
            );
            res.json({ success: true });
          }
        );
      ...
      db.run(
        "UPDATE users SET username = ?, isAdmin = ?, canUseDvr = ? WHERE id = ?",
        [username, isAdmin ? 1 : 0, canUseDvr ? 1 : 0, id],
        (err) => {
          if (err) {
            console.error(
              `[USER_API] Error updating user ${id} without password change:`,
              err.message
            );
            return res.status(500).json({ error: err.message });
          }
          ...
          console.log(
            `[USER_API] User ${id} updated successfully (without password change).`
          );
          res.json({ success: true });
        }
      );
  ...
  if (req.session.userId == id && !isAdmin) {
    console.log(
      `[USER_API] Attempting to demote current admin user ${id}. Checking if last admin.`
    );
    db.get(
      "SELECT COUNT(*) as count FROM users WHERE isAdmin = 1",
      [],
      (err, row) => {
        if (err) {
          console.error(
            "[USER_API] Error checking admin count for demotion:",
            err.message
          );
          return res.status(500).json({ error: err.message });
        }
        if (row.count <= 1) {
          console.warn(
            `[USER_API] Cannot demote user ${id}: They are the last administrator.`
          );
          return res
            .status(403)
...
app.delete("/api/users/:id", requireAdmin, (req, res) => {
  ...
  if (req.session.userId == idToDelete) {
    console.warn(
      `[USER_API] Attempted to delete own account for user ${idToDelete}.`
    );
    return res
      .status(403)
  ...
  for (const [streamKey, streamInfo] of activeStreamProcesses.entries()) {
    if (streamInfo.userId === idToDelete) {
      console.log(
        `[USER_DELETION] Found active stream for deleted user ${idToDelete}. Terminating PID: ${streamInfo.process.pid}.`
      );
      try {
        streamInfo.process.kill("SIGKILL");
        activeStreamProcesses.delete(streamKey);
        ...
        console.warn(
          `[USER_DELETION] Error killing stream process for user ${idToDelete}: ${e.message}`
        );
      }
    }
  }
  if (streamsKilled > 0) {
    console.log(
      `[USER_DELETION] Terminated ${streamsKilled} active stream(s) for deleted user ${idToDelete}.`
    );
  }

  // --- NEW: Force logout via SSE ---
  sendSseEvent(idToDelete, "force-logout", {
  ...
  db.run("DELETE FROM users WHERE id = ?", idToDelete, function (err) {
    if (err) {
      console.error(
        `[USER_API] Error deleting user ${idToDelete}:`,
        err.message
      );
      return res.status(500).json({ error: err.message });
    }
    ...
    console.log(
      `[USER_API] User ${idToDelete} deleted successfully from database.`
    );
    res.json({ success: true });
  });
});
...
app.get("/api/config", requireAuth, (req, res) => {
  try {
    ...
    if (fs.existsSync(MERGED_EPG_JSON_PATH)) {
      try {
        config.epgContent = JSON.parse(
          fs.readFileSync(MERGED_EPG_JSON_PATH, "utf-8")
        );
        console.log(`[API] Loaded EPG content from ${MERGED_EPG_JSON_PATH}.`);
      } catch (parseError) {
        console.error(
          `[API] Error parsing merged EPG JSON from ${MERGED_EPG_JSON_PATH}: ${parseError.message}`
        );
        config.epgContent = {};
      }
    } else {
      console.log(
        `[API] No merged EPG JSON file found at ${MERGED_EPG_JSON_PATH}.`
      );
    }
        ...
        if (rows) {
          ...
          rows.forEach((row) => {
            try {
              ...
              console.warn(
                `[API] User setting key "${row.key}" could not be parsed as JSON. Storing as raw string.`
              );
            }
          });
          ...
          console.log(
            `[API] Merged user settings for user ID: ${req.session.userId}`
          );
        }
    ...
    res
      .status(500)
      .json({ error: "Could not load configuration from server." });
  }
});
...
app.post(
  ...
  async (req, res) => {
    ...
    console.log(
      `[SOURCES_API] ${
        id ? "Updating" : "Adding"
      } source. Type: ${sourceType}, Name: ${name}`
    );
    ...
    if (!sourceType || !name) {
      ...
      console.warn(
        "[SOURCES_API] Source type or name missing for source operation."
      );
      return res
        .status(400)
        .json({ error: "Source type and name are required." });
    }

    const settings = getSettings();
    const sourceList =
      sourceType === "m3u" ? settings.m3uSources : settings.epgSources;
    ...
    if (id) {
      ...
      console.log(
        `[SOURCES_API] Found existing source for update: ${sourceToUpdate.name}`
      );
      ...
      if (req.file) {
        console.log(
          `[SOURCES_API] New file uploaded for source ${id}. Deleting old file if exists.`
        );
        if (
          sourceToUpdate.type === "file" &&
          fs.existsSync(sourceToUpdate.path)
        ) {
          try {
            fs.unlinkSync(sourceToUpdate.path);
            console.log(
              `[SOURCES_API] Deleted old file: ${sourceToUpdate.path}`
            );
          } catch (e) {
            console.error("[SOURCES_API] Could not delete old source file:", e);
          }
        }
        const extension = sourceType === "m3u" ? ".m3u" : ".xml";
        const newPath = path.join(
          SOURCES_DIR,
          `${sourceType}_${id}${extension}`
        );
        try {
          ...
          return res
            .status(500)
            .json({ error: "Could not save updated file." });
        }
        ...
        if (
          ...
          try {
            fs.unlinkSync(sourceToUpdate.path);
            console.log(
              `[SOURCES_API] Deleted old file (switching to URL): ${sourceToUpdate.path}`
            );
          } catch (e) {
            console.error(
              "[SOURCES_API] Could not delete old source file (on type change):",
              e
            );
          }
        }
        ...
        sourceToUpdate.type === "file" &&
        !req.file &&
        (!sourceToUpdate.path || !fs.existsSync(sourceToUpdate.path))
      ) {
        console.warn(
          `[SOURCES_API] Existing file source ${id} has no file and no new file/URL provided.`
        );
        return res
          .status(400)
          .json({
            error:
              "Existing file source requires a new file if original is missing.",
          });
      }
      ...
      if (newSource.type === "url" && !newSource.path) {
        ...
        return res
          .status(400)
          .json({ error: "URL is required for URL-type source." });
      }
      if (newSource.type === "file" && !req.file) {
        console.warn(
          "[SOURCES_API] New file source failed: A file must be selected."
        );
        return res
          .status(400)
          .json({
            error: "A file must be selected for new file-based sources.",
          });
      }
      ...
      if (req.file) {
        ...
        const newPath = path.join(
          SOURCES_DIR,
          `${sourceType}_${newSource.id}${extension}`
        );
        try {
          fs.renameSync(req.file.path, newPath);
          newSource.path = newPath;
          console.log(
            `[SOURCES_API] Renamed uploaded file for new source to: ${newPath}`
          );
        } catch (e) {
          console.error("[SOURCES_API] Error renaming new source file:", e);
          return res
            .status(500)
            .json({ error: "Could not save uploaded file." });
        }
      }
      ...
      saveSettings(settings);
      console.log(
        `[SOURCES_API] New source "${name}" added successfully (ID: ${newSource.id}).`
      );
      res.json({
        success: true,
...
app.put("/api/sources/:sourceType/:id", requireAuth, (req, res) => {
  ...
  console.log(
    `[SOURCES_API] Partial update source ID: ${id}, Type: ${sourceType}, isActive: ${isActive}`
  );
...
app.delete("/api/sources/:sourceType/:id", requireAuth, (req, res) => {
  ...
  if (source && source.type === "file" && fs.existsSync(source.path)) {
    try {
      ...
      console.error(
        `[SOURCES_API] Could not delete source file: ${source.path}`,
        e
      );
    }
  }
...
app.post("/api/process-sources", requireAuth, async (req, res) => {
  console.log(
    "[API] Received request to /api/process-sources (manual trigger)."
  );
  try {
    ...
    if (result.success) {
      ...
      console.log(
        "[API] Source processing completed and settings saved (manual trigger)."
      );
      res.json({ success: true, message: "Sources merged successfully." });
    } else {
      res
        .status(500)
        .json({ error: result.message || "Failed to process sources." });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: "Failed to process sources. Check server logs." });
  }
});
  ...
  try {
    ...
    for (const key in req.body) {
      if (
        ...
        console.warn(
          `[SETTINGS_SAVE] Attempted to save user-specific key "${key}" to global settings. This is ignored.`
        );
      }
    }
    ...
    res
      .status(500)
      .json({ error: "Could not save settings. Check server logs." });
  }
});
// ... existing endpoint ...
app.post("/api/user/settings", requireAuth, (req, res) => {
  ...
  const saveAndRespond = () => {
    ...
    db.all(
      ...
      (err, rows) => {
        if (err) {
          console.error(
            "[API] Error re-fetching user settings after save:",
            err
          );
          return res
            .status(500)
            .json({ error: "Could not retrieve updated settings." });
        }
        const userSettings = {};
        rows.forEach((row) => {
          try {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          `[API] Error saving user setting for user ${userId}, key ${key}:`,
          err
        );
        return res.status(500).json({ error: "Could not save user setting." });
      }
      console.log(
        `[API] User setting for user ${userId}, key ${key} saved successfully.`
      );
      saveAndRespond();
    }
  );
...
app.get("/api/notifications/vapid-public-key", requireAuth, (req, res) => {
  ...
  if (!vapidKeys.publicKey) {
    ...
    return res
      .status(500)
      .json({ error: "VAPID public key not available on the server." });
  }
...
app.post("/api/notifications/subscribe", requireAuth, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          `[PUSH_API] Error saving push subscription for user ${userId}:`,
          err
        );
        return res.status(500).json({ error: "Could not save subscription." });
      }
      console.log(
        `[PUSH] User ${userId} subscribed with endpoint: ${endpoint}. (ID: ${
...
app.post("/api/notifications/unsubscribe", requireAuth, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          `[PUSH_API] Error deleting push subscription for user ${req.session.userId}, endpoint ${endpoint}:`,
          err
        );
      ...
      if (this.changes === 0) {
        console.warn(
          `[PUSH_API] No subscription found for user ${req.session.userId} with endpoint ${endpoint} for deletion.`
        );
        return res
          .status(404)
          .json({ error: "Subscription not found or unauthorized." });
      }
      console.log(
        `[PUSH] User ${req.session.userId} unsubscribed from endpoint: ${endpoint}`
      );
      res.json({ success: true });
    }
  );
...
app.post("/api/notifications", requireAuth, (req, res) => {
  ...
  if (
    ...
    console.error(
      `[PUSH_API_ERROR] Add notification failed for user ${userId} due to missing data.`,
      { body: req.body }
    );
    return res
      .status(400)
      .json({
        error:
          "Invalid notification data. All required fields must be provided.",
      });
  }

  console.log(
    `[PUSH_API] Adding notification for user ${userId}. Program: "${programTitle}", Channel: "${channelName}", Scheduled Time: ${scheduledTime}`
  );

  db.run(
    `INSERT INTO notifications (user_id, channelId, channelName, channelLogo, programTitle, programDesc, programStart, programStop, notificationTime, programId, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
    [
      userId,
      channelId,
    ...
    function (err) {
      if (err) {
        console.error(
          `[PUSH_API_ERROR] Database error adding notification for user ${userId}:`,
          err
        );
        return res
          .status(500)
          .json({ error: "Could not add notification to the database." });
      }
      const notificationId = this.lastID;
      console.log(
        `[PUSH_API] Notification added successfully for program "${programTitle}" (DB ID: ${notificationId}) for user ${userId}.`
      );
      ...
      db.all(
        ...
        (subErr, subs) => {
          if (subErr) {
            console.error(
              `[PUSH_API_ERROR] Could not fetch subscriptions for user ${userId} to create deliveries.`,
              subErr
            );
            return;
          }
          ...
          const deliveryStmt = db.prepare(
            "INSERT INTO notification_deliveries (notification_id, subscription_id, status, updatedAt) VALUES (?, ?, 'pending', ?)"
          );
          subs.forEach((sub) => {
          ...
          deliveryStmt.finalize((finalizeErr) => {
            if (finalizeErr)
              console.error(
                `[PUSH_API_ERROR] Error finalizing delivery creation for notification ${notificationId}.`,
                finalizeErr
              );
            else
              console.log(
                `[PUSH_API] Created ${subs.length} delivery records for notification ${notificationId}.`
              );
          });
        }
      );
...
app.get("/api/notifications", requireAuth, (req, res) => {
  console.log(
    `[PUSH_API] Fetching notifications for user ${req.session.userId}.`
  );
  const query = `
        SELECT
            n.id,
  ...
  db.all(query, [req.session.userId], (err, rows) => {
    if (err) {
      console.error(
        "[PUSH_API] Error fetching consolidated notifications from database:",
        err
      );
      return res
        .status(500)
        .json({ error: "Could not retrieve notifications." });
    }
    console.log(
      `[PUSH_API] Found ${rows.length} consolidated notifications for user ${req.session.userId}.`
    );
    res.json(rows);
  });
});
...
app.delete("/api/notifications/past", requireAuth, (req, res) => {
  ...
  db.run(
    `DELETE FROM notifications WHERE user_id = ? AND notificationTime <= ?`,
    [userId, now],
    function (err) {
      if (err) {
        console.error(
          `[PUSH_API] Error deleting past notifications for user ${userId}:`,
          err.message
        );
        return res
          .status(500)
          .json({ error: "Could not clear past notifications." });
      }
      console.log(
        `[PUSH_API] Cleared ${this.changes} past notifications for user ${userId}.`
      );
...
app.delete("/api/notifications/:id", requireAuth, (req, res) => {
  const { id } = req.params;
  console.log(
    `[PUSH_API] Deleting notification ID: ${id} for user ${req.session.userId}.`
  );
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          `[PUSH_API] Error deleting notification ${id} from database:`,
          err
        );
        return res
          .status(500)
          .json({ error: "Could not delete notification." });
      }
      if (this.changes === 0) {
        console.warn(
          `[PUSH_API] Notification ${id} not found or unauthorized for user ${req.session.userId}.`
        );
        return res
          .status(404)
      ...
      console.log(
        `[PUSH_API] Notification ${id} deleted successfully for user ${req.session.userId}.`
      );
      res.json({ success: true });
    }
  );
...
app.delete("/api/data", requireAuth, requireAdmin, (req, res) => {
  console.log(
    `[API_RESET] ADMIN ACTION: Received request to /api/data (HARD RESET) from admin ${req.session.username}.`
  );
  try {
    ...
    db.serialize(() => {
      tables.forEach((table) => {
        db.run(`DELETE FROM ${table}`, (err) => {
          if (err)
            console.error(
              `[API_RESET] Error clearing table ${table}:`,
              err.message
            );
        });
      });
    ...
    console.log(
      "[API_RESET] Hard reset complete. The application is now in a fresh-install state."
    );
    res.json({
      success: true,
...
app.get("/stream", requireAuth, async (req, res) => {
  ...
  if (activeStreamInfo) {
    ...
    console.log(
      `[STREAM] Existing stream requested. Key: ${streamKey}. New ref count: ${activeStreamInfo.references}.`
    );
    activeStreamInfo.process.stdout.pipe(res);

    req.on("close", () => {
      console.log(
        `[STREAM] Client closed connection for existing stream ${streamKey}. Decrementing ref count.`
      );
      activeStreamInfo.references--;
      activeStreamInfo.lastAccess = Date.now();
      if (activeStreamInfo.references <= 0) {
        console.log(
          `[STREAM] Last client disconnected. Ref count is 0. Process for PID: ${activeStreamInfo.process.pid} will be cleaned up by the janitor.`
        );
      }
    });
    return;
  ...
  console.log(
    `[STREAM] New request: URL=${streamUrl}, ProfileID=${profileId}, UserAgentID=${userAgentId}`
  );
  if (!streamUrl)
    return res.status(400).send("Error: `url` query parameter is required.");

  let settings = getSettings();
  const profile = (settings.streamProfiles || []).find(
    (p) => p.id === profileId
  );

  if (!profile) {
    console.error(
      `[STREAM] Stream profile with ID "${profileId}" not found in settings.`
    );
    return res
      .status(404)
      .send(`Error: Stream profile with ID "${profileId}" not found.`);
  }
  ...
  const userAgent = (settings.userAgents || []).find(
    (ua) => ua.id === userAgentId
  );
  if (!userAgent) {
    console.error(
      `[STREAM] User agent with ID "${userAgentId}" not found in settings.`
    );
    return res
      .status(404)
      .send(`Error: User agent with ID "${userAgentId}" not found.`);
  }

  //-- ENHANCEMENT: Find channel name and logo for logging.
  const allChannels = parseM3U(
    fs.existsSync(MERGED_M3U_PATH)
      ? fs.readFileSync(MERGED_M3U_PATH, "utf-8")
      : ""
  );
  ...
  console.log(
    `[STREAM] Using Profile='${profile.name}' (ID=${profile.id}), UserAgent='${userAgent.name}'`
  );
  ...
  const args = (commandTemplate.match(/(?:[^\s"]+|"[^"]*")+/g) || []).map(
    (arg) => arg.replace(/^"|"$/g, "")
  );
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          "[STREAM_HISTORY] Error logging stream start:",
          err.message
        );
      } else {
        const historyId = this.lastID;
        console.log(
          `[STREAM_HISTORY] Logged stream start with history ID: ${historyId}`
        );
        ...
        console.log(
          `[STREAM] Started FFMPEG process with PID: ${ffmpeg.pid} for user ${userId} for stream key: ${streamKey}.`
        );
  ...
  ffmpeg.stderr.on("data", (data) =>
    console.error(
      `[FFMPEG_ERROR] Stream: ${streamKey} - ${data.toString().trim()}`
    )
  );
  ...
  const cleanupOnExit = () => {
    ...
    if (info && info.historyId) {
      const endTime = new Date().toISOString();
      const duration = Math.round(
        (new Date(endTime).getTime() - new Date(info.startTime).getTime()) /
          1000
      );
      db.run(
        "UPDATE stream_history SET end_time = ?, duration_seconds = ?, status = 'stopped' WHERE id = ? AND status = 'playing'",
  ...
  ffmpeg.on("close", (code) => {
    console.log(
      `[STREAM] ffmpeg process for ${streamKey} exited with code ${code}`
    );
    cleanupOnExit();
    if (!res.headersSent)
      res
        .status(500)
        .send("FFmpeg stream ended unexpectedly or failed to start.");
    else res.end();
  });
  ffmpeg.on("error", (err) => {
    console.error(
      `[STREAM] Failed to start ffmpeg process for ${streamKey}: ${err.message}`
    );
    cleanupOnExit();
    if (!res.headersSent)
      res
        .status(500)
        .send("Failed to start streaming service. Check server logs.");
  });
  ...
  req.on("close", () => {
    ...
    if (info) {
      console.log(
        `[STREAM] Client closed connection for new stream ${streamKey}. Decrementing ref count.`
      );
      info.references--;
      info.lastAccess = Date.now();
      if (info.references <= 0) {
        console.log(
          `[STREAM] Last client disconnected. Ref count is 0. Process for PID: ${info.process.pid} will be cleaned up by the janitor.`
        );
      }
    } else {
      console.log(
        `[STREAM] Client closed connection for ${streamKey}, but no process was found in the map.`
      );
    }
  });
});
...
app.post("/api/stream/stop", requireAuth, (req, res) => {
  ...
  if (!streamUrl) {
    return res
      .status(400)
      .json({ error: "Stream URL is required to stop the stream." });
  }
  ...
  if (activeStreamInfo) {
    console.log(
      `[STREAM_STOP_API] Received request to stop stream for user ${req.session.userId}. Terminating key: ${streamKey}`
    );
    try {
      if (activeStreamInfo.historyId) {
        ...
        const duration = Math.round(
          (new Date(endTime).getTime() -
            new Date(activeStreamInfo.startTime).getTime()) /
            1000
        );
        db.run(
          "UPDATE stream_history SET end_time = ?, duration_seconds = ?, status = 'stopped' WHERE id = ? AND status = 'playing'",
      ...
      broadcastAdminUpdate();
      console.log(
        `[STREAM_STOP_API] Successfully terminated process for key: ${streamKey}`
      );
    } catch (e) {
      console.warn(
        `[STREAM_STOP_API] Could not kill process for key: ${streamKey}. It might have already exited. Error: ${e.message}`
      );
    }
    res.json({
      success: true,
      message: `Stream process for ${streamKey} terminated.`,
    });
  } else {
    console.log(
      `[STREAM_STOP_API] Received stop request for user ${req.session.userId}, but no active stream was found for key: ${streamKey}`
    );
...
app.post("/api/activity/start-redirect", requireAuth, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(
          "[REDIRECT_LOG] Error logging redirect stream start:",
          err.message
        );
      ...
      const historyId = this.lastID;
      console.log(
        `[REDIRECT_LOG] Logged redirect stream start for user ${username} with history ID: ${historyId}`
      );
...
app.post("/api/activity/stop-redirect", requireAuth, (req, res) => {
  ...
  const endTime = new Date().toISOString();
  db.get(
    "SELECT start_time FROM stream_history WHERE id = ? AND user_id = ?",
    ...
    (err, row) => {
      if (err || !row) {
        ...
        return res
          ...
          .json({
            success: true,
            message: "Stream stopped, history record not found.",
          });
      }
      const duration = Math.round(
        (new Date(endTime).getTime() - new Date(row.start_time).getTime()) /
          1000
      );
      db.run(
        "UPDATE stream_history SET end_time = ?, duration_seconds = ?, status = 'stopped' WHERE id = ? AND end_time IS NULL",
        [endTime, duration, historyId],
        (updateErr) => {
          if (updateErr) {
            console.error(
              `[REDIRECT_LOG] Error updating redirect stream end for history ID ${historyId}:`,
              updateErr.message
            );
            return res.status(500).json({ error: "Could not log stream end." });
          }
          console.log(
            `[REDIRECT_LOG] Logged redirect stream end for history ID: ${historyId}`
          );
          res.json({ success: true });
        }
      );
...
app.get("/api/admin/activity", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (search) {
      whereClauses.push(
        `(username LIKE ? OR channel_name LIKE ? OR client_ip LIKE ? OR stream_profile_name LIKE ?)`
      );
    ...
    const countResult = await new Promise((resolve, reject) => {
      db.get(
        `SELECT COUNT(*) as total FROM stream_history ${whereString}`,
        queryParams,
        (err, row) => {
          if (err) reject(err);
          else resolve(row);
    ...
    console.error(
      "[ADMIN_API] Error fetching paginated stream history:",
      err.message
    );
    return res
      .status(500)
      .json({ error: "Could not retrieve stream history." });
  }
});
...
app.post("/api/admin/stop-stream", requireAuth, requireAdmin, (req, res) => {
  ...
  if (!streamKey) {
    return res
      .status(400)
      .json({ error: "A streamKey is required to stop the stream." });
  }

  const streamInfo = activeStreamProcesses.get(streamKey);
  if (streamInfo) {
    console.log(
      `[ADMIN_API] Admin ${req.session.username} is terminating stream ${streamKey} for user ${streamInfo.username}.`
    );
    try {
      if (streamInfo.historyId) {
        const endTime = new Date().toISOString();
      ...
      res.json({
        success: true,
        message: `Stream terminated for user ${streamInfo.username}.`,
      });
    } catch (e) {
      console.error(
        `[ADMIN_API] Error terminating stream ${streamKey}: ${e.message}`
      );
...
app.post("/api/admin/change-stream", requireAuth, requireAdmin, (req, res) => {
  ...
  if (!userId || !streamKey || !channel) {
    return res
      .status(400)
      .json({ error: "User ID, stream key, and channel data are required." });
  }
  ...
  if (!streamInfo || streamInfo.userId !== userId) {
    return res
      .status(404)
      .json({ error: "The specified stream is not active for this user." });
  }

  console.log(
    `[ADMIN_API] Admin ${req.session.username} is changing channel for user ${streamInfo.username} to "${channel.name}".`
  );
  ...
  res.json({
    success: true,
    message: `Change channel command sent to user ${streamInfo.username}.`,
  });
});
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Could not retrieve system health information." });
    }
  }
);
...
app.get("/api/admin/analytics", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const topChannels = await new Promise((resolve, reject) => {
      db.all(topChannelsQuery, [], (err, rows) =>
        err ? reject(err) : resolve(rows)
      );
    });

    const topUsers = await new Promise((resolve, reject) => {
      db.all(topUsersQuery, [], (err, rows) =>
        err ? reject(err) : resolve(rows)
      );
    });
...
app.get("/api/multiview/layouts", requireAuth, (req, res) => {
  ...
  db.all(
    "SELECT id, name, layout_data FROM multiview_layouts WHERE user_id = ?",
    [req.session.userId],
    (err, rows) => {
      if (err) {
      ...
      console.log(
        `[LAYOUT_API] Found ${layouts.length} layouts for user ${req.session.userId}.`
      );
      res.json(layouts);
    }
  );
...
app.post("/api/multiview/layouts", requireAuth, (req, res) => {
  ...
  console.log(
    `[LAYOUT_API] Saving layout "${name}" for user ${req.session.userId}.`
  );
  if (!name || !layout_data) {
  ...
  db.run(
    "INSERT INTO multiview_layouts (user_id, name, layout_data) VALUES (?, ?, ?)",
    ...
    function (err) {
      ...
      console.log(
        `[LAYOUT_API] Layout "${name}" saved with ID ${this.lastID} for user ${req.session.userId}.`
      );
      res
        .status(201)
        .json({ success: true, id: this.lastID, name, layout_data });
    }
  );
});
...
app.delete("/api/multiview/layouts/:id", requireAuth, (req, res) => {
  const { id } = req.params;
  console.log(
    `[LAYOUT_API] Deleting layout ID: ${id} for user ${req.session.userId}.`
  );
  db.run(
    "DELETE FROM multiview_layouts WHERE id = ? AND user_id = ?",
    [id, req.session.userId],
    function (err) {
      if (err) {
      ...
      if (this.changes === 0) {
        console.warn(
          `[LAYOUT_API] Layout ${id} not found or user ${req.session.userId} not authorized.`
        );
        return res
          .status(404)
          .json({
            error:
              "Layout not found or you do not have permission to delete it.",
          });
      }
...
async function checkAndSendNotifications() {
  console.log(
    "[PUSH_CHECKER] Running scheduled notification check for all devices."
  );
  const now = new Date();
  ...
  try {
    db.run(
      `
        ...
        `,
      [nowIso, timeoutCutoff],
      function (err) {
        if (err) {
          console.error(
            "[PUSH_CHECKER_CLEANUP] Error expiring old notifications:",
            err.message
          );
        } else if (this.changes > 0) {
          console.log(
            `[PUSH_CHECKER_CLEANUP] Expired ${this.changes} old notification deliveries.`
          );
        }
      }
    );
    ...
    if (dueDeliveries.length > 0) {
      console.log(
        `[PUSH_CHECKER] Found ${dueDeliveries.length} due notification deliveries to process.`
      );
    } else {
      return;
    }
    ...
    for (const delivery of dueDeliveries) {
      console.log(
        `[PUSH_CHECKER] Processing delivery ID ${delivery.delivery_id} for program "${delivery.programTitle}" to subscription ${delivery.subscription_id}.`
      );
      ...
      const payload = JSON.stringify({
        ...
        data: {
          ...
          channelLogo:
            delivery.channelLogo || "https://i.imgur.com/rwa8SjI.png",
      ...
      webpush
        .sendNotification(pushSubscription, payload, pushOptions)
        .then(() => {
          console.log(
            `[PUSH_CHECKER] Successfully sent notification for delivery ID ${delivery.delivery_id}.`
          );
          db.run(
            "UPDATE notification_deliveries SET status = 'sent', updatedAt = ? WHERE id = ?",
            [nowIso, delivery.delivery_id]
          );
        })
        .catch((error) => {
          console.error(
            `[PUSH_CHECKER] Error sending notification for delivery ID ${delivery.delivery_id}:`,
            error.statusCode,
          ...
          if (error.statusCode === 410 || error.statusCode === 404) {
            console.log(
              `[PUSH_CHECKER] Subscription ${delivery.subscription_id} is invalid (410/404). Deleting subscription and failing deliveries.`
            );
            ...
            db.run("DELETE FROM push_subscriptions WHERE id = ?", [
              delivery.subscription_id,
            ]);
            db.run(
              "UPDATE notification_deliveries SET status = 'failed', updatedAt = ? WHERE subscription_id = ? AND status = 'pending'",
              [nowIso, delivery.subscription_id]
            );
          } else {
            db.run(
              "UPDATE notification_deliveries SET status = 'failed', updatedAt = ? WHERE id = ?",
              [nowIso, delivery.delivery_id]
            );
          }
        });
    ...
    console.error(
      "[PUSH_CHECKER] Unhandled error in checkAndSendNotifications:",
      error
    );
  }
}
...
function stopRecording(jobId) {
  ...
  if (pid) {
    console.log(
      `[DVR] Gracefully stopping recording for job ${jobId} (PID: ${pid}). Sending SIGINT.`
    );
    try {
      process.kill(pid, "SIGINT");
    } catch (e) {
      console.error(
        `[DVR] Error sending SIGINT to ffmpeg process for job ${jobId}: ${e.message}. Trying SIGKILL.`
      );
      try {
        process.kill(pid, "SIGKILL");
      } catch (e2) {}
    ...
    console.warn(
      `[DVR] Cannot stop job ${jobId}: No running ffmpeg process found.`
    );
  }
}
...
async function startRecording(job) {
  console.log(
    `[DVR] Starting recording for job ${job.id}: "${job.programTitle}"`
  );
  const settings = getSettings();
  const allChannels = parseM3U(
    fs.existsSync(MERGED_M3U_PATH)
  ...
  if (!channel) {
    ...
    db.run(
      "UPDATE dvr_jobs SET status = 'error', ffmpeg_pid = NULL, errorMessage = ? WHERE id = ?",
      [errorMsg, job.id]
    );
    return;
  }
  ...
  const recProfile = (settings.dvr.recordingProfiles || []).find(
    (p) => p.id === job.profileId
  );
  if (!recProfile) {
  ...
  const safeFilename = `${job.id}_${job.programTitle
    .replace(/[^a-z0-9]/gi, "_")
  ...
  console.log(
    `[DVR] Spawning ffmpeg for job ${job.id} with command: ffmpeg ${args.join(
      " "
    )}`
  );
  const ffmpeg = spawn("ffmpeg", args);
  ...
  db.run(
    "UPDATE dvr_jobs SET status = 'recording', ffmpeg_pid = ?, filePath = ? WHERE id = ?",
    [ffmpeg.pid, fullFilePath, job.id]
  );
  ...
  ffmpeg.on("close", (code) => {
    ...
    const wasStoppedIntentionally = ffmpegErrorOutput.includes(
      "Exiting normally, received signal 2"
    );
    const logMessage =
      code === 0 || wasStoppedIntentionally
        ? "finished gracefully"
        : `exited with error code ${code}`;
    console.log(
      `[DVR] Recording process for job ${job.id} ("${job.programTitle}") ${logMessage}.`
    );
    ...
    fs.stat(fullFilePath, (statErr, stats) => {
      if (
        ...
        db.run(
          `INSERT INTO dvr_recordings (job_id, user_id, channelName, programTitle, startTime, durationSeconds, fileSizeBytes, filePath) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            job.id,
            job.user_id,
          ...
          (insertErr) => {
            if (insertErr) {
              console.error(
                `[DVR] Failed to create dvr_recordings entry for job ${job.id}:`,
                insertErr.message
              );
            } else {
              console.log(
                `[DVR] Job ${job.id} logged to completed recordings.`
              );
            }
          }
        );
        db.run(
          "UPDATE dvr_jobs SET status = 'completed', ffmpeg_pid = NULL WHERE id = ?",
          [job.id]
        );
      } else {
        const finalErrorMessage = `Recording failed. FFmpeg exit code: ${code}. ${
          statErr ? "File stat error: " + statErr.message : ""
        }. FFmpeg output: ${ffmpegErrorOutput.slice(-1000)}`;
        console.error(
          `[DVR] Recording for job ${job.id} failed. ${finalErrorMessage}`
        );
        db.run(
          "UPDATE dvr_jobs SET status = 'error', ffmpeg_pid = NULL, errorMessage = ? WHERE id = ?",
          [finalErrorMessage, job.id]
        );
        if (!statErr && stats.size <= 1024) {
          fs.unlink(fullFilePath, (unlinkErr) => {
            if (unlinkErr)
              console.error(
                `[DVR] Could not delete failed recording file: ${fullFilePath}`,
                unlinkErr
              );
          });
        }
...
function scheduleDvrJob(job) {
  ...
  if (endTime <= now) {
    console.log(
      `[DVR] Job ${job.id} for "${job.programTitle}" is already in the past. Skipping schedule.`
    );
    if (job.status === "scheduled") {
      db.run(
        "UPDATE dvr_jobs SET status = 'error', errorMessage = 'Job was scheduled for a time in the past.' WHERE id = ?",
        [job.id]
      );
    }
    return;
  ...
  if (startTime > now) {
    ...
    console.log(
      `[DVR] Scheduled recording start for job ${job.id} at ${startTime}`
    );
  } else {
    startRecording(job);
  }
...
function loadAndScheduleAllDvrJobs() {
  console.log(
    "[DVR] Loading and scheduling all pending DVR jobs from database..."
  );
  db.run(
    "UPDATE dvr_jobs SET status = 'error', errorMessage = 'Server restarted during recording.' WHERE status = 'recording'",
    [],
    (err) => {
      if (err) {
        console.error(
          "[DVR] Error updating recording jobs status on startup:",
          err.message
        );
      } else {
        console.log(
          "[DVR] Updated status of previous 'recording' jobs to 'error'."
        );
      }

      db.all(
        "SELECT * FROM dvr_jobs WHERE status = 'scheduled'",
        [],
        (err, jobs) => {
          if (err) {
...
async function checkForConflicts(newJob, userId) {
  return new Promise((resolve, reject) => {
    ...
    db.all(
      "SELECT * FROM dvr_jobs WHERE user_id = ? AND status = 'scheduled'",
      [userId],
      (err, scheduledJobs) => {
...
async function autoDeleteOldRecordings() {
  console.log(
    "[DVR_STORAGE] Running daily check for old recordings to delete."
  );
  db.all("SELECT id FROM users", [], (err, users) => {
    if (err)
      return console.error(
        "[DVR_STORAGE] Could not fetch users for auto-delete check:",
        err
      );

    users.forEach((user) => {
      db.get(
        "SELECT value FROM user_settings WHERE user_id = ? AND key = 'dvr'",
        [user.id],
        (err, row) => {
          const settings = getSettings();
          const userDvrSettings = row
            ? { ...settings.dvr, ...JSON.parse(row.value) }
            : settings.dvr;
          ...
          db.all(
            "SELECT id, filePath FROM dvr_recordings WHERE user_id = ? AND startTime < ?",
            [user.id, cutoffDate.toISOString()],
            (err, recordingsToDelete) => {
              if (err)
                return console.error(
                  `[DVR_STORAGE] Error fetching old recordings for user ${user.id}:`,
                  err
                );
              if (recordingsToDelete.length > 0) {
                console.log(
                  `[DVR_STORAGE] Found ${recordingsToDelete.length} old recording(s) to delete for user ${user.id}.`
                );
              }
              ...
              recordingsToDelete.forEach((rec) => {
                if (fs.existsSync(rec.filePath)) {
                  fs.unlink(rec.filePath, (unlinkErr) => {
                    if (unlinkErr) {
                      console.error(
                        `[DVR_STORAGE] Failed to delete file ${rec.filePath}:`,
                        unlinkErr
                      );
                    } else {
                      db.run("DELETE FROM dvr_recordings WHERE id = ?", [
                        rec.id,
                      ]);
                      console.log(
                        `[DVR_STORAGE] Deleted old recording file and DB record: ${rec.filePath}`
                      );
                    }
                  });
                } else {
...
app.get(
  ...
  (req, res) => {
    ...
    const userId = req.session.userId;
    console.log(
      `[DVR_TIMESHIFT] Received request for job ${jobId} from user ${userId}.`
    );
    ...
    db.get(
      ...
      (err, job) => {
        ...
        if (!job) {
          return res
            .status(404)
            .send("Recording job not found or not authorized.");
        }
        if (job.status !== "recording") {
          return res
            .status(400)
            .send("Cannot timeshift a recording that is not in progress.");
        }
        ...
        stream.on("error", (streamErr) => {
          console.error(
            `[DVR_TIMESHIFT] Error streaming file ${job.filePath}:`,
            streamErr
          );
          res.end();
        });
...
app.post(
  ...
  requireDvrAccess,
  async (req, res) => {
    const { channelId, channelName, programTitle, programStart, programStop } =
      req.body;
    const settings = getSettings();
    ...
    const newJob = {
      ...
      programTitle,
      startTime: new Date(
        new Date(programStart).getTime() - preBuffer
      ).toISOString(),
      endTime: new Date(
        new Date(programStop).getTime() + postBuffer
      ).toISOString(),
      status: "scheduled",
    ...
    if (conflictingJobs.length > 0) {
      return res
        .status(409)
        .json({
          error: "Recording conflict detected.",
          newJob,
          conflictingJobs,
        });
    }
    ...
    db.run(
      ...
      [
        ...
        newJob.profileId,
        newJob.userAgentId,
        newJob.preBufferMinutes,
        newJob.postBufferMinutes,
      ],
      function (err) {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Could not schedule recording." });
        }
  ...
  if (req.session.isAdmin) {
    ...
    db.all(query, [], (err, rows) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Failed to retrieve all recording jobs." });
      res.json(rows);
    });
  } else if (req.session.canUseDvr) {
    // Users with DVR access see only their own jobs
    const query =
      "SELECT * FROM dvr_jobs WHERE user_id = ? ORDER BY startTime DESC";
    db.all(query, [req.session.userId], (err, rows) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Failed to retrieve your recording jobs." });
      // Add a username to be consistent with admin view
      const jobsWithUser = rows.map((r) => ({
        ...r,
        username: req.session.username,
      }));
...
app.get("/api/dvr/recordings", requireAuth, (req, res) => {
  ...
  const query =
    "SELECT r.*, u.username FROM dvr_recordings r JOIN users u ON r.user_id = u.id ORDER BY r.startTime DESC";
  db.all(query, [], (err, rows) => {
    if (err)
      return res.status(500).json({ error: "Failed to retrieve recordings." });
    const recordingsWithFilename = rows.map((r) => ({
      ...r,
  ...
  try {
    disk.check(DVR_DIR, (err, info) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Could not get storage information." });
      }
...
app.delete("/api/dvr/jobs/all", requireAuth, requireDvrAccess, (req, res) => {
  const userId = req.session.userId;
  console.log(
    `[DVR_API] Clearing all scheduled/historical jobs for user ${userId}.`
  );

  db.all(
    "SELECT id FROM dvr_jobs WHERE user_id = ? AND status = 'scheduled'",
    [userId],
    (err, scheduledJobs) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Could not fetch jobs to cancel." });
      }
      ...
      db.run(
        "DELETE FROM dvr_jobs WHERE user_id = ?",
        [userId],
        function (err) {
          if (err) {
            return res
              .status(500)
              .json({ error: "Could not clear jobs from database." });
          }
          res.json({ success: true, deletedCount: this.changes });
        }
      );
...
app.delete(
  "/api/dvr/recordings/all",
  requireAuth,
  requireDvrAccess,
  (req, res) => {
    const userId = req.session.userId;
    console.log(
      `[DVR_API] Deleting all completed recordings for user ${userId}.`
    );

    db.all(
      "SELECT id, filePath FROM dvr_recordings WHERE user_id = ?",
      [userId],
      (err, recordings) => {
        if (err) {
          return res
            .status(500)
            .json({ error: "Could not fetch recordings to delete." });
        }
        ...
        recordings.forEach((rec) => {
          if (fs.existsSync(rec.filePath)) {
            fs.unlink(rec.filePath, (unlinkErr) => {
              if (unlinkErr)
                console.error(
                  `[DVR_API] Failed to delete file ${rec.filePath}:`,
                  unlinkErr
                );
            });
          }
        ...
        db.run(
          "DELETE FROM dvr_recordings WHERE user_id = ?",
          [userId],
          function (err) {
            if (err) {
              return res
                .status(500)
                .json({ error: "Could not clear recordings from database." });
            }
            res.json({ success: true, deletedCount: this.changes });
          }
        );
...
app.delete("/api/dvr/jobs/:id", requireAuth, requireDvrAccess, (req, res) => {
  ...
  db.run(query, params, function (err) {
    ...
    if (this.changes === 0)
      return res
        .status(404)
        .json({ error: "Job not found or not authorized to cancel." });
  ...
  (req, res) => {
    ...
    db.get(query, params, (err, row) => {
      if (err || !row)
        return res
          .status(404)
          .json({ error: "Recording not found or not authorized." });

      if (fs.existsSync(row.filePath)) {
        fs.unlink(row.filePath, (unlinkErr) => {
          if (unlinkErr)
            console.error(
              `[DVR_API] Failed to delete file ${row.filePath}:`,
              unlinkErr
            );
        });
      }
      db.run("DELETE FROM dvr_recordings WHERE id = ?", [id], (deleteErr) => {
        if (deleteErr)
          return res
            .status(500)
            .json({ error: "Failed to delete recording record." });
        res.json({ success: true });
      });
    });
  }
...
app.post(
  ...
  (req, res) => {
    ...
    const jobId = parseInt(id, 10);
    console.log(
      `[DVR_API] Received request to stop recording for job ${jobId}.`
    );
    stopRecording(jobId);
    ...
    db.run(query, params, function (err) {
      if (err)
        return res
          .status(500)
          .json({ error: "Could not update job status after stop." });
      if (this.changes === 0)
        return res
          .status(404)
          .json({ error: "Job not found or not authorized to stop." });
      res.json({ success: true });
    });
  }
);
...
app.put("/api/dvr/jobs/:id", requireAuth, requireDvrAccess, (req, res) => {
  ...
  const getQuery = req.session.isAdmin
  ...
  db.get(getQuery, getParams, (err, job) => {
    ...
    if (job.status !== "scheduled")
      return res
        .status(400)
        .json({ error: "Only scheduled jobs can be modified." });

    db.run(
      "UPDATE dvr_jobs SET startTime = ?, endTime = ? WHERE id = ?",
      ...
      if (["error", "cancelled", "completed"].includes(job.status)) {
        db.run("DELETE FROM dvr_jobs WHERE id = ?", [id], function (err) {
          if (err)
            return res
              .status(500)
              .json({ error: "Could not delete job history." });
        ...
        return res
          ...
          .json({
            error:
              "Only completed, cancelled, or error jobs can be removed from history.",
          });
      }
    });
  }
...
app.get("/api/events", requireAuth, (req, res) => {
  ...
  console.log(
    `[SSE] Client ${clientId} connected for user ID ${userId}. Total clients for user: ${clients.length}.`
  );
  ...
  req.on("close", () => {
    ...
    if (userClients) {
      ...
      if (index !== -1) {
        ...
        console.log(
          `[SSE] Client ${clientId} disconnected for user ID ${userId}. Remaining clients for user: ${userClients.length}.`
        );
...
app.post("/api/validate-url", requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "URL is reachable and returned a successful response.",
    });
  } catch (error) {
    res
      .status(400)
      .json({
        success: false,
        error: `URL is not reachable. Error: ${error.message}`,
      });
  }
});
...
app.get("/api/public-ip", requireAuth, (req, res) => {
  ...
  https
    .get("https://ifconfig.me/ip", (ipRes) => {
      let data = "";
      ipRes.on("data", (chunk) => {
        data += chunk;
      });
...
app.post(
  "/api/settings/import",
  requireAdmin,
  settingsUpload.single("settingsFile"),
  (req, res) => {
    if (!req.file) {
    ...
    try {
      ...
      console.log(
        "[SETTINGS_IMPORT] Settings file imported successfully. App will now use new settings."
      );
      res.json({
        success: true,
        message:
          "Settings imported. The application will use them on next load.",
      });
    } catch (error) {
      console.error(
        "[SETTINGS_IMPORT] Error processing imported settings file:",
        error.message
      );
      if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
      res
        .status(400)
        .json({ error: `Invalid settings file. Error: ${error.message}` });
    }
  }
);
// --- Main Route Handling ---
...
function parseM3U(data) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("#EXTINF:")) {
      ...
      if (
        nextLine &&
        (nextLine.startsWith("http") || nextLine.startsWith("rtp"))
      ) {
